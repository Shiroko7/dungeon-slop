...
import { RefineResponseSchema, applyRefineOps, describeOp } from "../engine/refine-ops.ts";
...
function parseDataUrl(input: string): AIImage | null {
  const match = /^data:(image\/[a-zA-Z0-9.+-]+);base64,([A-Za-z0-9+/=]+)$/.exec(input.trim());
...
export async function handleRefineLayout(req: Request): Promise<Response> {
  ...
  const parsedImage =
    aiProvider.supportsImages === true && typeof image === "string" ? parseDataUrl(image) : null;
    ...
    async start(controller) {
      ...
      try {
        ...
        if (!validation.success) {
          ...
          controller.enqueue(
            encoder.encode(
              sseEvent("error", { error: `Refinement failed validation: ${errors.join("; ")}` }),
            ),
          );
          controller.close();
          return;
        ...
        const { blueprint: refined, results } = applyRefineOps(blueprint, validation.data.ops);
